        ...
        <div class="grid gap-3 lg:grid-cols-[minmax(220px,1fr)_220px_auto]">
            ...
            <label class="block text-sm font-medium text-slate-700">Filter
                <select wire:model.live="filter" class="mt-1 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm">
                    ...
                    @if($tab === 'ggb')
                        <option value="material">Materi RPP</option><option value="heading">Subjudul (tidak dijadwalkan)</option><option value="artifact">Artefak sumber (tidak dijadwalkan)</option><option value="section">Bagian sumber</option><option value="item">Butir sumber</option>
                    @elseif($tab === 'syllabus')
        ...
        <div class="spreadsheet-scroll max-h-[68vh] overflow-auto" x-ref="grid">
            <table class="spreadsheet-table" data-grid-table>
                ...
                <tr>
                    ...
                    @if($tab === 'ggb')
                        <th class="sticky-column-id" aria-sort="{{ $sortField === 'stable_code' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('stable_code')">Kode stabil</button></th><th aria-sort="{{ $sortField === 'aspect' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('aspect')">Aspek</button></th><th aria-sort="{{ $sortField === 'subaspect' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('subaspect')">Subaspek</button></th><th aria-sort="{{ $sortField === 'title' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('title')">Materi</button></th><th aria-sort="{{ $sortField === 'rpp_role' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('rpp_role')">Peran RPP</button></th><th>Target</th><th aria-sort="{{ $sortField === 'sort_order' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('sort_order')">Urutan</button></th><th>Sumber</th>
                    @elseif($tab === 'syllabus')
                ...
                @forelse($rows as $row)
                    <tr data-grid-row="{{ $row->id }}">
                        ...
                        @if($tab === 'ggb')
                            ...
                            <td><select data-grid-cell data-field="rpp_role" data-id="{{ $row->id }}" data-version="{{ $row->lock_version }}"><option value="material" @selected($row->rpp_role === 'material')>Materi</option><option value="heading" @selected($row->rpp_role === 'heading')>Subjudul</option><option value="artifact" @selected($row->rpp_role === 'artifact')>Artefak sumber</option></select><p class="mt-1 text-[11px] text-slate-500">{{ $row->rpp_role_source === 'manual' ? 'Ditetapkan Admin' : 'Saran struktur sumber' }}</p></td>
        ...
        @forelse($rows as $row)
            <details class="panel group p-4">
                ...
                <div class="mobile-row-editor mt-4 space-y-3">
                    @if($tab === 'ggb')
                        @foreach(['aspect'=>'Aspek','subaspect'=>'Subaspek','title'=>'Materi','target_text'=>'Target','sort_order'=>'Urutan'] as $field => $label)<label>{{ $label }}<textarea data-grid-cell data-field="{{ $field }}" data-id="{{ $row->id }}" data-version="{{ $row->lock_version }}">{{ $row->{$field} }}</textarea></label>@endforeach<label>Peran RPP<select data-grid-cell data-field="rpp_role" data-id="{{ $row->id }}" data-version="{{ $row->lock_version }}"><option value="material" @selected($row->rpp_role === 'material')>Materi</option><option value="heading" @selected($row->rpp_role === 'heading')>Subjudul</option><option value="artifact" @selected($row->rpp_role === 'artifact')>Artefak sumber</option></select></label><p class="text-xs text-slate-500">Subjudul dan artefak tetap tersimpan untuk audit, tetapi tidak masuk cakupan atau RPP.</p>
                    @elseif($tab === 'syllabus')
