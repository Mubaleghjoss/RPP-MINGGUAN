        ...
        <div class="spreadsheet-scroll max-h-[68vh] overflow-auto" x-ref="grid">
            <table class="spreadsheet-table" data-grid-table>
                ...
                <tr>
                    ...
                    @elseif($tab === 'syllabus')
                        <th class="sticky-column-id" aria-sort="{{ $sortField === 'stable_code' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('stable_code')">Kode stabil</button></th><th aria-sort="{{ $sortField === 'category' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('category')">Kategori</button></th><th aria-sort="{{ $sortField === 'title' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('title')">Materi</button></th><th>Penjabaran</th><th>Alokasi</th><th aria-sort="{{ $sortField === 'recommended_sessions' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('recommended_sessions')">Pertemuan</button></th><th>Referensi</th><th>Penilaian</th><th>Duplikat</th><th aria-sort="{{ $sortField === 'semester_scope' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('semester_scope')">Semester efektif</button></th><th>Semester sumber</th><th aria-sort="{{ $sortField === 'sort_order' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('sort_order')">Urutan</button></th><th>Sumber</th>
                    @elseif($tab === 'link')
