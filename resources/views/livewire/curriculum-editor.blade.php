        ...
        <div class="grid gap-3 lg:grid-cols-[minmax(220px,1fr)_220px_auto]">
            ...
            <label class="block text-sm font-medium text-slate-700">Filter
                <select wire:model.live="filter" class="mt-1 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm">
                    ...
                    @elseif($tab === 'syllabus')
                        <option value="allocation">Perlu alokasi</option><option value="duplicate">Duplikat</option>
                    @elseif($tab === 'link')
        ...
        <div class="spreadsheet-scroll max-h-[68vh] overflow-auto" x-ref="grid">
            <table class="spreadsheet-table" data-grid-table>
                ...
                <tr>
                    ...
                    @elseif($tab === 'syllabus')
                        <th class="sticky-column-id" aria-sort="{{ $sortField === 'stable_code' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('stable_code')">Kode stabil</button></th><th aria-sort="{{ $sortField === 'category' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('category')">Kategori</button></th><th aria-sort="{{ $sortField === 'title' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('title')">Materi</button></th><th>Penjabaran</th><th>Alokasi</th><th aria-sort="{{ $sortField === 'recommended_sessions' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('recommended_sessions')">Pertemuan</button></th><th>Referensi</th><th>Penilaian</th><th>Duplikat</th><th aria-sort="{{ $sortField === 'sort_order' ? ($sortDirection === 'asc' ? 'ascending' : 'descending') : 'none' }}"><button wire:click="sortBy('sort_order')">Urutan</button></th><th>Sumber</th>
                    @elseif($tab === 'link')
        ...
        @forelse($rows as $row)
            <details class="panel group p-4">
                ...
                <div class="mobile-row-editor mt-4 space-y-3">
                    ...
                    @elseif($tab === 'syllabus')
                        @foreach(['category'=>'Kategori','title'=>'Materi','description'=>'Penjabaran','allocation_text'=>'Alokasi','recommended_sessions'=>'Pertemuan','reference_text'=>'Referensi','assessment_text'=>'Penilaian','sort_order'=>'Urutan'] as $field => $label)<label>{{ $label }}<textarea data-grid-cell data-field="{{ $field }}" data-id="{{ $row->id }}" data-version="{{ $row->lock_version }}">{{ $row->{$field} }}</textarea></label>@endforeach<label>Duplikat<select data-grid-cell data-field="is_duplicate" data-id="{{ $row->id }}" data-version="{{ $row->lock_version }}"><option value="0" @selected(!$row->is_duplicate)>Tidak</option><option value="1" @selected($row->is_duplicate)>Ya</option></select></label>
                    @elseif($tab === 'link')
