<div x-data="spreadsheetGrid('rpp')" data-grid-domain="rpp" x-on:keydown.window="handleShortcut($event)" x-on:focus-validation-field.window="$nextTick(() => window.focusValidationField($event.detail.field))" class="preview-grid space-y-6">
    ...
    <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-8" aria-label="Ringkasan semester">
        ...
        <a href="{{ route('exports.index', ['level' => $plan->level_id, 'semester' => $semester, 'detail' => 'gaps']) }}#matrix-gaps" class="panel block cursor-pointer p-4 transition-colors duration-150 hover:border-emerald-400 hover:bg-emerald-50/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600"><p class="text-sm text-slate-500">Kelengkapan Matriks</p><p class="metric-number mt-2">{{ number_format($matrixStats['percent'], 1) }}%</p><p class="mt-1 text-xs text-slate-500">{{ $matrixStats['filled'] }}/{{ $matrixStats['total'] }} sel terisi · {{ $matrixStats['missing'] ? $matrixStats['missing'].' perlu isian' : 'Lengkap' }}</p></a>
    </section>
    ...
    @elseif($detail === 'gaps')
        @include('livewire.partials.export-matrix-gaps')
    @elseif($detail === 'activities')
        @include('livewire.partials.export-activities')
    @endif
    ...
    <section class="panel p-4 shadow-sm lg:sticky lg:top-3 lg:z-30" aria-label="Toolbar preview">
        <div class="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
            <div class="flex flex-wrap gap-2">
                ...
                <button type="button" wire:click="showDetail('activities')" class="button-secondary">Atur Kegiatan</button>
            ...
            <aside class="absolute inset-y-0 right-0 flex w-full max-w-2xl flex-col bg-white shadow-2xl">
                <div class="border-b border-slate-200 p-5 sm:p-6">
                    ...
                    <div class="mt-3 flex flex-wrap gap-2" aria-label="Filter status materi">
                        @foreach(['all' => 'Semua', 'activity' => 'Bank Kegiatan', 'unused' => 'Belum Masuk', 'used' => 'Sudah Terpasang', 'week' => 'Minggu Ini', 'unmapped' => 'Belum Dipetakan'] as $value => $label)
                ...
                <div class="border-t border-slate-200 bg-white p-4 sm:p-5">
                    <details class="mb-4 rounded-xl bg-slate-50 p-3 ring-1 ring-slate-200"><summary class="min-h-11 cursor-pointer py-2 font-semibold text-slate-900">Isian manual satu kali</summary><div class="mt-3 grid gap-3"><label class="grid gap-1 text-sm font-medium text-slate-700">Isi kegiatan<textarea wire:model="manualActivityContent" rows="3" class="rounded-xl border border-slate-300 bg-white px-3 py-2" placeholder="Contoh: Kegiatan kebersihan kelas"></textarea></label><label class="grid gap-1 text-sm font-medium text-slate-700">Alasan tindakan<input wire:model="manualActivityReason" class="min-h-11 rounded-xl border border-slate-300 bg-white px-3" placeholder="Minimal 5 karakter"></label><button type="button" wire:click="addOneOffActivity" wire:loading.attr="disabled" wire:target="addOneOffActivity" class="button-secondary">Tambahkan Manual & Kunci</button></div></details>
                    <label class="grid gap-1 text-sm font-medium text-slate-700">Alasan penambahan<input wire:model="pickerReason" type="text" class="min-h-11 rounded-xl border border-slate-300 px-3" placeholder="Minimal 5 karakter"></label>
                    @if($errorMessage)<p class="mt-2 text-sm font-medium text-red-700" role="alert">{{ $errorMessage }}</p>@endif
                    <div class="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><span class="text-sm font-medium text-slate-600"><span class="font-mono tabular-nums">{{ count($pickerSelected) }}</span> materi dipilih</span><button type="button" wire:click="addSelectedMaterials" wire:loading.attr="disabled" wire:target="addSelectedMaterials" class="button-primary" @disabled(count($pickerSelected) === 0)><span wire:loading.remove wire:target="addSelectedMaterials">Tambahkan & Kunci</span><span wire:loading wire:target="addSelectedMaterials">Menambahkan…</span></button></div>
                </div>
            </aside>
        </div>
    @endif
